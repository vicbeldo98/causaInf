...
from causal_effect import (compute_estimands, compute_estimation_methods,
                           estimate_effect_with_estimand_and_estimator,
                           estimate_with_variables, refuting_tests)
...
def compute_effect_from_graph():
    ...
    causal_effect = estimate_with_variables(
        session["csv_content"],
        request.form["graph"],
        request.form["treatment"],
        request.form["outcome"],
        request.form["adjusted"],
    )
    return {
        "treatment": request.form["treatment"],
        "outcome": request.form["outcome"],
        "effect": round(causal_effect.value, 3),
    }
...
def compute_effect_with_estimand_and_estimator():
    estimate = estimate_effect_with_estimand_and_estimator(
        session["model"],
        session["identified_estimand"],
...
def refutation():
    return dict(
        refuting_tests(
            session["model"], session["identified_estimand"], session["estimate"]
        )
    )
